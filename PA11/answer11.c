...
data and manipulate it with specific functions. For example, you can
...
/*
* Return TRUE if 'state' is a valid puzzle state.
*
...
int isValidState(const char * state)
{
  ...
  char temp[len+1];
  int m;
  for (m = 0; m < 17; m++){temp[m] = state[m];}
  qsort(temp, len, sizeof(char), sort);
  if(strcmp(temp , "-123456789ABCDEF") != 0){return FALSE;}
  return TRUE;
}

/**
* Return TRUE if all characters in 'moves' are in "RLUD"
*
                         ...
                         const char * moves)
{
  if (node == NULL)
    {
      MoveTree *new = MoveTree_create(state, moves);
      return new;
    }
...
void generateAllHelper(MoveTree *root, int n_moves, const char *state, char *movelist, int ind)
{
  ...
  char *possible_moves = "RLUD";
  int num_possible_moves = sizeof(possible_moves) / sizeof(char);
  int i;
  for (i = 0; i < num_possible_moves; i++)
    {
      char m = possible_moves[i];
...
char * solve(char * state)
{
  ...
  MoveTree *final = MoveTree_find(tree, FINAL_STATE);
  char *movelist = NULL;

  if(final != NULL)
    {
      movelist = strdup(final->moves);
    }
