...
{
  int i = 0;//loop controll variable
  while(s[i] != '\0')
    {
      if(s[i] >='A' && s[i] <= 'Z')
	{
...
const char *my_strstr(const char * s1, const char * s2)
{
  ...
  int n; //loop control variable
  int length_2; //the length of s2
  
  length_2 = my_strlen(s2); 
  for(j = 0; s1 != '\0'; j++)
    { 
      n = 0;
      if(s1[j] == s2[0])
...
void my_strinsert(char *s1, const char *s2, int pos)
{
  ...
  int length_1;//the length of string s1
  int length_2;//the length of string s2
  int i;//loop control variable
  int j;//loop control variable
  length_1 = my_strlen(s1);
  length_2 = my_strlen(s2);
  if(length_1 <= pos)
    {
      my_strcat(s1,s2); 
...
{
  int s_len;//the length of string s
  int i;//loop control variable
  ...
  if(s_len <= (pos+length) && s_len > pos)
    ...
    {
      for(i = 0; i < s_len - (pos+length);i++)
	{ 
	  s[pos + i] = s[i + pos + length];
	}   
      s[s_len - length] = '\0';
    }
}
